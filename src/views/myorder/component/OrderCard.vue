<template>
  <div class="flex items-center text-xs">
    <p>订单号：{{ msg.id }}</p>
    <p class="text-red-400 ml-auto">订单状态：{{ msg.orderstate }}</p>
  </div>
  <div v-for="(item, index) in msg.orderproducts" :key="index" class="mt-4 space-y-4">
  ...
  <p v-if="msg.ipaysum" class="text-right mt-4 text-xs text-gray-600">{{ count }}件商品  共{{ msg.ipaysum }}港币</p>
</template>
