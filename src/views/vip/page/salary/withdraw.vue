  ...
  <div class="relative pt-12 bg-white min-h-screen">
    ...
    <div class="px-4 py-3.5 space-y-3.5">
      ...
      >
        <div class="flex items-center text-sm">
          <p>提现收益(港币)</p>
          <p class="ml-auto text-gray-600">余额：{{ balance }}港币</p>
        </div>
        <div class="flex items-center space-x-4">
          <p class="font-bold leading-none flex-shrink-0 text-gray-600">HK$</p>
          <p class="text-sm" :class="+balance > 0 ? 'text-4xl font-bold' : 'text-gray-500'">
            {{ +balance > 0 ? balance : '当前余额不能转出' }} <span v-if="+balance > 0" class="text-base">港币</span>
          </p>
        </div>
        <p class="text-sm">预计七个工作日内到账</p>
      </div>
      ...
      >
        ...
        <div v-if="!title" class="px-2 py-1">
    ...
    <div class="mt-3 space-y-2.5 px-4">
      ...
      <div class="text-xs space-y-1.5">
        <p>高级经理每次提现收费标准如下：</p>
        <p>高级经理每次提现收取手续费50元/笔（人民币）。</p>
        <p>提现金额超过5000港币（含5000）时免手续费。</p>
        <p>由于木子网采用港币结算，提现到个人账户为人民币，具体到账金额按实时汇率计算。</p>
        <p>高级经理的收益在提取时，由木子网代为缴纳个人所得税，具体税额按照《中华人民共和国个人所得税法》相关规定缴纳。</p>
      </div>
    </div>
    <div class="mt-6 px-9">
    ...
    <van-popup v-model:show="showPopup" round closeable :style="{ width: '90%' }">
      <div class="px-4 py-6 bg-gray-200 h-full relative">
        <p class="text-center text-lg font-bold">{{ title ? '编辑银行卡' : '添加银行卡' }}</p>
  ...
  setup() {
    ...
    api.get('/newB/getTiXian').then((res) => {
      if(res.data.code === 20000) {
        if(+res.data.data.balance > 50) btnDisabled.value = false
        ...
        if(res.data.data.kaihuBank && res.data.data.cardnum) {
          title.value = res.data.data.kaihuBank + '(' + res.data.data.cardnum.slice(-4) +')'
        }
      }
    })
    return {
      ...
      // 提现
      withdraw() {
        if(!title.value) { Toast.fail('请添加本人银行卡'); return }
