...
export default {
  ...
  setup() {
    ...
    const onAdd = () => {
      router.push({ path: '/vip/address/edit', query: { operation: 'creat', from: route.fullPath, addressLength: list.value.length } })
    }
    const onEdit = (item) => {
      router.push({ path: '/vip/address/edit', query: { operation: 'edit', from: route.fullPath, addressId: item.id, addressLength: list.value.length } })
    }
    const onSelect = (item) => {
      chosenAddressId.value = item.id
      sessionStorage.setItem('addressId',item.id)
      Toast.success('选择成功')
      router.replace('/vip/confirmorder?from='+route.query.from)
    }
    return {
      switchable,
      showLoading,
